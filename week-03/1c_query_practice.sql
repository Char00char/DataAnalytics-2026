...
SELECT ShipCountry, COUNT(DISTINCT CustomerID) AS CustomerCount FROM orders
...
-- Write a query to list each of the job titles in employees, along with a count of how many employees hold each job title. Year Up United Data Analyst Training Academy Week 3 Lab Workbook
SELECT Title, COUNT(*) AS EmployeeCount FROM employees
GROUP BY Title;
-- What employees have a monthly salary that is between $2000 and $2500? Write a query that orders them by job title.
SELECT EmployeeID, FirstName, LastName, Title, Salary FROM employees
